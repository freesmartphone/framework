...
str_to_type = {'int' : int, 'bool' : bool, 'str' : str, 'var': object, 'dict': list}
...
class MyApp(object):
    def __init__(self):
        ...
        bus = dbus.SystemBus()
        conf_manager = bus.get_object("org.freesmartphone.opreferencesd", "/org/freesmartphone/Preferences")
        ...
        profile_combobox = self.w_tree.get_widget('profile_combobox')
        profile = conf_manager.GetProfiles()
        for p in profile:
            profile_combobox.append_text(p)
    ...
    def on_service_combobox_changed(self, w):
        ...
        self.service = bus.get_object("org.freesmartphone.opreferencesd", service_path)
        ...
        for key in keys[:]:
            ...
            if type_str == 'dict':
                keys.remove(key)
                values = self.service.GetValue(key)
                for v in values:
                    keys.append('%s/%s' % (key, str(v)))
