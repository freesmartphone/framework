...
AUTOPREFIX = re.compile( "AT(?P<prefix>[^=?;\r\n]*)" )
...
#=========================================================================#
    ...
    def enqueue( self, data, response_cb=None, error_cb=None, prefixes=None ):
        ...
        if prefixes is None:
            match = AUTOPREFIX.match( data )
            if match is not None:
                prefix = match.groupdict()["prefix"]
                logger.debug( "%s: Autogenerated prefix for command %s: %s", self, repr(data), prefix )
                prefixes = [ prefix ]
            else:
                logger.warning( "%s: Can't autogenerate prefix for command %s", self, repr(data) )
                prefixes = []
